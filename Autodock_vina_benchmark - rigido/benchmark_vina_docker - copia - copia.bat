...
for /l %%i in (1, 1, %RUNS%) do (
    ...
    if errorlevel 1 (
        ...
        :: Convertir PDBQT a PDB con Open Babel
        "%OBABEL%" -i pdbqt "%WORKDIR%\output_%%i.pdbqt" -o pdb -O "%WORKDIR%\output_%%i.pdb" >nul 2>&1
        
        :: Calcular RMSD usando Python + RDKit
        echo from rdkit import Chem > "%WORKDIR%\calc_rmsd_temp.py"
        echo from rdkit.Chem import AllChem >> "%WORKDIR%\calc_rmsd_temp.py"
        echo. >> "%WORKDIR%\calc_rmsd_temp.py"
        echo try: >> "%WORKDIR%\calc_rmsd_temp.py"
        echo     ref = Chem.MolFromPDBFile('ligand_exp.pdb', removeHs=True^) >> "%WORKDIR%\calc_rmsd_temp.py"
        echo     probe = Chem.MolFromPDBFile('output_%%i.pdb', removeHs=True^) >> "%WORKDIR%\calc_rmsd_temp.py"
        ...
        echo         ref = Chem.RemoveHs(ref^) >> "%WORKDIR%\calc_rmsd_temp.py"
        echo         probe = Chem.RemoveHs(probe^) >> "%WORKDIR%\calc_rmsd_temp.py"
        echo         rmsd = AllChem.GetBestRMS(ref, probe^) >> "%WORKDIR%\calc_rmsd_temp.py"
        ...
        echo     print(f"Error calculando RMSD: {e}"^) >> "%WORKDIR%\calc_rmsd_temp.py"
