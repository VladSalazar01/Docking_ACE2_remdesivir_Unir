...
def extraer_coordenadas_pdbqt(ruta_pdbqt):
    """Extrae coordenadas XYZ de átomos pesados de un archivo PDBQT."""
    coordenadas = []
    nombres_atomos = []
...
def ejecutar_vina_docker(nombre_sitio, config, run_id, archivo_log):
    ...
    # Mapeo de carpeta de trabajo a /data en el contenedor
    cmd = [
        "docker", "run", "--rm",
    ...
    # Extraer afinidad del log o output
...
def calcular_rmsd_run(ruta_salida_pdbqt, ruta_referencia_pdb, archivo_log):
    ...
    log(f"  [2/3] Calculando RMSD (Kabsch)...", archivo_log)
    ...
    # Si el número de átomos pesados difiere, usar el mínimo
    n_ref = len(coords_ref)
    n_dock = len(coords_dock)
    
    if n_ref != n_dock:
        log(f"  [AVISO] Átomos pesados: ref={n_ref}, dock={n_dock} (usando min={min(n_ref, n_dock)})", archivo_log)
    
    rmsd = calcular_rmsd_kabsch(coords_dock, coords_ref)
    
    if rmsd >= 0:
        log(f"  📊 RMSD: {rmsd:.3f} Å", archivo_log)
    else:
...
def comparacion_dual_site(stats_sitios, archivo_log):
    ...
    # Comparación con benchmark original (punto MID)
    log("", archivo_log)
    log("=" * 70, archivo_log)
    log("📋 COMPARACIÓN CON BENCHMARK ORIGINAL (PUNTO MID)", archivo_log)
    log("=" * 70, archivo_log)
    log(f"  Centro original (MID):  ({CENTRO_MID_INCORRECTO['center_x']}, "
        f"{CENTRO_MID_INCORRECTO['center_y']}, {CENTRO_MID_INCORRECTO['center_z']})", archivo_log)
    log(f"  RMSD original:          4.446 ± 0.140 Å (exh=64, n=5)", archivo_log)
    log(f"  ⚠️ {CENTRO_MID_INCORRECTO['nota']}", archivo_log)
    log("", archivo_log)
...
def main():
    ...
    log("", archivo_log)
    log("  CORRECCIÓN: Centro de caja cambiado de punto MID a fragmentos", archivo_log)
