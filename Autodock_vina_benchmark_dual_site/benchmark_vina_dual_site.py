...
# Carpeta de trabajo
CARPETA_TRABAJO = Path(r"G:\Autodock_vina_benchmark_dual_site")
...
def ejecutar_vina_docker(nombre_sitio, config, run_id, archivo_log):
    ...
    log(f"  [OK] Docking completado en {duracion:.1f}s", archivo_log)
...
def calcular_rmsd_run(ruta_salida_pdbqt, ruta_referencia_pdb, archivo_log):
    ...
    if rmsd >= 0:
        log(f"  [STATS] RMSD: {rmsd:.3f} Å ({n_pares} pares name-matched)", archivo_log)
    else:
...
def ejecutar_benchmark_sitio(nombre_sitio, config, n_runs, archivo_log):
    ...
    log("", archivo_log)
    log("=" * 70, archivo_log)
    log(f"[SITIO] BENCHMARKING SITIO: {nombre_sitio}", archivo_log)
...
def calcular_estadisticas(resultados, nombre_sitio, archivo_log):
    ...
    log("", archivo_log)
    log(f"[STATS] ESTADÍSTICAS {nombre_sitio} (n={n_exitosos}/{n_total}):", archivo_log)
    log("-" * 50, archivo_log)
    
    if n_exitosos == 0:
        log(f"  [AVISO] Sin runs exitosos para {nombre_sitio}", archivo_log)
        return None
...
def tabla_resultados_por_run(resultados, nombre_sitio, archivo_log):
    ...
    for r in resultados:
        ...
        estado = "[OK]" if r["exito"] else "[FAIL]"
...
def comparacion_dual_site(stats_sitios, archivo_log):
    ...
    log("", archivo_log)
    log("=" * 70, archivo_log)
    log("[STATS] COMPARACIÓN DUAL-SITE", archivo_log)
    log("=" * 70, archivo_log)
    
    sitios_validos = {k: v for k, v in stats_sitios.items() if v is not None}
    
    if len(sitios_validos) == 0:
        log("  [AVISO] Sin resultados válidos para comparar", archivo_log)
        return
    ...
    if len(sitios_validos) == 2:
        ...
        if s1 and s2:
            ...
            log("", archivo_log)
            log(f"[RESULTADO] MEJOR SITIO: {mejor}", archivo_log)
    ...
    log("", archivo_log)
    log("=" * 70, archivo_log)
    log("[COMPARACION] COMPARACIÓN CON BENCHMARK ORIGINAL", archivo_log)
    log("=" * 70, archivo_log)
    ...
    for nombre, stats in sitios_validos.items():
        ...
        if rmsd <= 1.0:
            ...
            calificacion = "[OK] EXCELENTE"
        elif rmsd <= 3.0:
            calificacion = "[--] ACEPTABLE"
        else:
            calificacion = "[FAIL] INSUFICIENTE"
...
def main():
    ...
    # Banner
    log("=" * 70, archivo_log)
    log("[BENCHMARK] AUTODOCK VINA - BENCHMARK DUAL-SITE (CORRECCIÓN RETROSPECTIVA)", archivo_log)
    log("=" * 70, archivo_log)
    ...
    log("", archivo_log)
    log(f"[TIEMPO] TIEMPO TOTAL BENCHMARK: {min_total}m {seg_total}s ({duracion_global:.1f}s)", archivo_log)
    ...
    log(f"\n[ARCHIVOS] Resultados guardados en:", archivo_log)
    ...
    log("\n[OK] Benchmark dual-site completado.", archivo_log)
